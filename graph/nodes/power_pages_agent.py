"""
Power Pages Agent node: Extracts features for Power Pages components using LLM.
"""
from typing import Any
from graph.utils.utils import save_to_cache, clean_llm_json, load_from_cache
import yaml
import os
import json
from graph.schemas.strategic_overview import AppComponent
from graph.utils.llm import call_llm
from rich.console import Console
from graph.utils.log import log_info, log_success, log_warning, log_error

console = Console()

def load_power_pages_prompt():
    PROMPT_PATH = os.path.join(os.path.dirname(__file__), "../prompts/power_pages_agent.yaml")
    with open(PROMPT_PATH, 'r', encoding='utf-8') as f:
        return yaml.safe_load(f)

def power_pages_agent(state: Any) -> Any:
    """
    Processes a single Power Pages component, extracting features using LLM.
    Updates the state with the new features for that component.
    """
    solution = getattr(state, "strategic_context", None)
    if not solution:
        log_error("[PowerPagesAgent] No strategic context found in state.")
        return state
    mvp_components = getattr(solution, "mvp_components", [])
    comp = None
    comp_index = None
    for idx, c in enumerate(mvp_components):
        if getattr(c, "app_type", None) == "PowerPages" and not getattr(c, "processed", False):
            comp = c
            comp_index = idx
            break
    if comp is None:
        log_warning("[PowerPagesAgent] No unprocessed Power Pages component found.")
        return state
    # Check for cached output
    cached = load_from_cache("power_pages_agent")
    if cached:
        log_success("[Cache] Using cached output for node 'power_pages_agent'")
        state.strategic_context = cached.get("strategic_context", state.strategic_context)
        mvp_components[comp_index].processed = True
        solution.mvp_components = mvp_components
        state.strategic_context = solution
        return state
    comp_dict = comp.model_dump() if hasattr(comp, "model_dump") else comp.__dict__
    parent_component = comp_dict.copy()
    parent_component.pop("app_screens", None)
    project_goal = getattr(solution, "purpose", "")
    business_value = getattr(solution, "business_value", [])
    summary = []
    prompt_yaml = load_power_pages_prompt()
    system_message = prompt_yaml.get("system_message", "")
    user_template = prompt_yaml["user_template"]
    chunk_content = ""
    user_prompt = user_template \
        .replace("{{ project_goal }}", project_goal) \
        .replace("{{ business_value }}", json.dumps(business_value, ensure_ascii=False)) \
        .replace("{{ solution_summary }}", json.dumps(summary, ensure_ascii=False, indent=2)) \
        .replace("{{ parent_component }}", json.dumps(parent_component, ensure_ascii=False, indent=2)) \
        .replace("{{ current_item }}", json.dumps(comp_dict, ensure_ascii=False, indent=2)) \
        .replace("{{ chunk_content }}", chunk_content)
    llm_response = call_llm(
        prompt=user_prompt,
        model="gpt-4-1106-preview",
        temperature=0.2,
        system_message=system_message
    )
    try:
        cleaned_response = clean_llm_json(llm_response)
        data = json.loads(cleaned_response)
        comp_dict["features"] = data.get("features", {})
    except Exception as e:
        log_error(f"[PowerPagesAgent] Failed to parse LLM output for Power Pages '{comp_dict.get('app_name', '')}': {e}")
        log_info(f"[PowerPagesAgent] Raw LLM response for Power Pages '{comp_dict.get('app_name', '')}':\n{llm_response}\n")
        comp_dict["features"] = {}
    if "app_screens" in comp_dict:
        with console.status("[bold blue]Power Pages Agent: Extracting features...", spinner="dots"):
            for sidx, screen in enumerate(comp_dict["app_screens"]):
                chunk_content = ""  # Optionally, implement a function to find relevant PRD chunk for the screen
                user_prompt = user_template \
                    .replace("{{ project_goal }}", project_goal) \
                    .replace("{{ business_value }}", json.dumps(business_value, ensure_ascii=False)) \
                    .replace("{{ solution_summary }}", json.dumps(summary, ensure_ascii=False, indent=2)) \
                    .replace("{{ parent_component }}", json.dumps(parent_component, ensure_ascii=False, indent=2)) \
                    .replace("{{ current_item }}", json.dumps(screen, ensure_ascii=False, indent=2)) \
                    .replace("{{ chunk_content }}", chunk_content)
                llm_response = call_llm(
                    prompt=user_prompt,
                    model="gpt-4-1106-preview",
                    temperature=0.2,
                    system_message=system_message
                )
                try:
                    cleaned_response = clean_llm_json(llm_response)
                    data = json.loads(cleaned_response)
                    if "features" in data and data["features"]:
                        screen["features"] = data["features"]
                except Exception as e:
                    log_error(f"[PowerPagesAgent] Failed to parse LLM output for screen '{screen.get('screen_name', '')}': {e}")
                    log_info(f"[PowerPagesAgent] Raw LLM response for screen '{screen.get('screen_name', '')}':\n{llm_response}\n")
    comp_dict["processed"] = True
    mvp_components[comp_index] = AppComponent(**comp_dict)
    solution.mvp_components = mvp_components
    state.strategic_context = solution
    save_to_cache("power_pages_agent", solution)
    log_success("Power Pages Agent: feature extraction complete.")
    return state
